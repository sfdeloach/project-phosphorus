...
export class UploadService {
  ...
  upload(fileContents: string, officers: Officer[]) {
    ...
    if (this.csvService.isValidFile('XCAD', tableArray[0])) {
      ...
      this.episodeHttpService.message.next(
        new Message(this.episodes.length + ' XCAD events processed. Ready to receive Cafe file.')
      );
    } else if (this.csvService.isValidFile('Cafe', tableArray[0]) && this.episodes.length > 0) {
      this.episodes = this.cafeService.addCafeData(this.episodes, tableArray, officers);
