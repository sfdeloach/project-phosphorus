...
export class UploadComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subOfficers = this.officerService.officers.subscribe((ofcs: Officer[]) => {
      this.messages.push(new Message('Ready to upload'));
      this.officers = ofcs;
    });

    this.subUploadMessages = this.uploadService.message.subscribe((message: Message) => {
      this.clearMessages();
      this.messages.push(message);
    });

    this.subEpisodeMessages = this.episodeService.message.subscribe((message: Message) => {
      this.clearMessages();
      this.messages.push(message);
    });
  }
  ...
  upload() {
    ...
    fileReader.onload = (evt: ProgressEvent) => {
      setTimeout(() => {
        this.uploadService.upload(fileReader.result as string, this.officers);
      }, 1000);
    };
  }
}
